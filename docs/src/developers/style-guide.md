...
!!! info
    The style guide is always a work in progress, and not all PyQHACompat code
    follows the rules. When modifying PyQHACompat, please fix the style violations
...
PyQHACompat uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/PyQHACompat.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the PyQHACompat directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to PyQHACompat have
