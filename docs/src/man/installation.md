...
[PyQHACompat](https://github.com/MineralsCloud/PyQHACompat.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("PyQHACompat")
   ```

3. Run

   ```julia-repl
   julia> using PyQHACompat
   ```
...
version of PyQHACompat, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/PyQHACompat.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/PyQHACompat.jl)
for new releases.
Once we release a new version, you can update PyQHACompat by typing

```@repl
using Pkg
Pkg.update("PyQHACompat")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("PyQHACompat")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall PyQHACompat.
